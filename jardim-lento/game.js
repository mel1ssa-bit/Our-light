(() => {
  ...
  // Logical pixel screen (Game Boy / pet vibe)
  const GW = 160;
  const GH = 140;
  const SCALE = 3;
  ...
  function drawScene() {
    ...
    if (world.rewardTicks > 0 && world.t % 8 < 4) {
  ...
  function tickWorld() {
    ...
    // bird — during reward: slow fly across the middle of the sky
    if (world.birdReward || world.rewardId === "bird") {
      world.birdDir = 1;
      world.birdX += 0.55;
      world.birdY = 20 + Math.sin(world.t / 12) * 4;
      if (world.birdX > GW + 10) world.birdX = -12;
    } else {
      world.birdX += 0.35 * world.birdDir;
      world.birdY = 18 + Math.sin(world.t / 18) * 3;
    ...
    if (world.butterflyOn) {
      world.butterflyX += 0.25;
      world.butterflyY = 44 + Math.sin(world.t / 10) * 5;
      if (world.butterflyX > GW + 8) world.butterflyOn = false;
    } else if (!world.rewardId && world.t % 280 === 0) {
    ...
    if (world.rewardId === "snail" || mode === "break") {
      ...
      world.snailX += world.rewardId === "snail" ? 0.28 : 0.14;
      if (world.snailX > GW) world.snailX = -10;
    } else if (state.plant && world.t % 450 < 140) {
      world.snailVisible = true;
      world.snailX += 0.1;
      if (world.snailX > GW) world.snailX = -10;
    } else {
    ...
    if (world.bugReward || world.rewardId === "bug") {
      ...
      world.bugOn = state.plant && state.stage >= 2 && world.t % 200 < 100;
    }
  ...
  function spawnReward(forcedId) {
    ...
    world.rewardTicks = 80; // ~8s at 10fps
  ...
  function startAmbience() {
    stopAmbience();
    // ~10 fps — pet 2000 / e-ink friendly
    animId = setInterval(tickWorld, 100);
    tickWorld();
  }
  ...
  function runTicker() {
    ...
    tickId = requestAnimationFrame(function loop(now) {
